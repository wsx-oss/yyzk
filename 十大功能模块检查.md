...
## 6. 十大模块：单机测试 vs 双机测试 需求分析

> **核查方法**：逐一检查每个模块的后端 API 实现（`api.go`）、数据采集源（`monitor.go`）、前端调用方式（`modules/*.html`），确认其数据来源是否依赖外部设备/网络连接。

### 6.1 总览表

| 模块 | 仅需 1 台电脑 | 需要 2 台电脑 | 判定依据 |
|------|:---:|:---:|------|
| 1. 系统状态监控 | ✅ | | `gopsutil` 采集**后端本机** CPU/内存/磁盘/网络 |
| 2. 语音交互记录 | ✅ | | 增删改查全在本机 SQLite + 本机文件系统 |
| 3. 远程桌面控制 | | ❌ | VNC 代理需要 `net.Dial("tcp", target)` 连接**目标机器的 VNC Server** |
| 4. 视频监控 | ✅ | | 前端 `getUserMedia` 调用**浏览器所在电脑的摄像头**，纯前端 |
| 5. 硬件状态检测 | ✅ | | `gopsutil` 采集**后端本机**硬件信息（主机名/OS/CPU型号/内存） |
| 6. 软件更新管理 | ✅ | | 更新记录存本机 SQLite `updates` 表，check 也查本机 DB |
| 7. 数据同步状态 | ✅ | | 读写本机 SQLite `sync_status` 表，无真实跨机器同步 |
| 8. 异常报警 | ✅ | | 报警记录存本机 SQLite `alerts` 表 |
| 9. 性能分析报告 | ✅ | | 汇总本机实时指标（`CollectMetrics`）+ 本机 DB 日志/告警计数 |
| 10. 维护操作日志 | ✅ | | 日志存本机 SQLite `logs` 表 |

**结论：9 个模块仅需 1 台电脑，只有"远程桌面控制"需要 2 台电脑。**
...
**模块 1：系统状态监控**
- 后端 `CollectMetrics()`（`monitor.go:44`）使用 `gopsutil` 的 `cpu.Percent`、`mem.VirtualMemory`、`disk.Usage`、`net.IOCounters` 等函数
- 这些函数采集的全部是**运行 Go 程序的这台电脑**的系统指标
- WebSocket 实时推送（`MetricsStream`）和 HTTP 快照（`MetricsSnapshot`）均不依赖外部设备
- **代码证据**：`monitor.go:46` `cpu.Percent(0, false)` → 采集本机 CPU

**模块 2：语音交互记录**
- 后端 API：`AudioUpload`/`AudioAdd`/`AudioList`/`AudioDownload`/`AudioDelete`
- 数据存储：本机 SQLite `recordings` 表 + 本机 `data/recordings/` 目录
- 所有操作（新增、删除、搜索、导入导出）均在本机完成
- **代码证据**：`api.go:270` `a.db.Exec(INSERT INTO recordings...)` → 写本机 DB

**模块 4：视频监控**
- 前端使用 `navigator.mediaDevices.getUserMedia({video:true})`
- 这是浏览器 API，调用的是**打开浏览器的这台电脑的摄像头**
- 完全不涉及后端 API，不需要外部摄像头或远程视频流
- **代码证据**：`video.html:182` `navigator.mediaDevices.getUserMedia(constraints)` → 本机摄像头
- **前提条件**：电脑需要有摄像头（笔记本内置摄像头即可）

**模块 5：硬件状态检测**
- 后端 `HardwareInfo()`（`monitor.go:70`）使用 `host.Info()`、`cpu.Info()`、`mem.VirtualMemory()`
- 采集的是后端本机的主机名、操作系统、CPU 型号、内存总量等
- **代码证据**：`monitor.go:73` `host.Info()` → 本机硬件信息

**模块 6：软件更新管理**
- 后端 API：`UpdatesList`/`UpdatesAdd`/`UpdatesCheck`
- 全部操作本机 SQLite `updates` 表
- `UpdatesCheck` 查询本机 DB 中最新版本号进行比对，不访问外部更新源
- **代码证据**：`api.go:439` `a.db.QueryRow(SELECT version FROM updates...)` → 查本机 DB

**模块 7：数据同步状态**
- 后端 API：`SyncStatusGet`/`SyncStatusSet`
- 读写本机 SQLite `sync_status` 表的 status/message/last_synced_at 字段
- "立即同步"只是修改状态字段值，**不涉及真实的跨机器数据传输**
- **代码证据**：`api.go:458` `UPDATE sync_status SET status=?...WHERE id=1` → 写本机 DB

**模块 8：异常报警**
- 后端 API：`AlertsList`/`AlertAck`/`AlertNew`
- 全部操作本机 SQLite `alerts` 表
- **代码证据**：`api.go:373` `INSERT INTO alerts(category, severity, message)...` → 写本机 DB

**模块 9：性能分析报告**
- 后端 `ReportPerf`（`api.go:463`）做两件事：
  1. 调用 `monitor.CollectMetrics()` 获取本机实时指标
  2. 查询本机 DB 中 `logs` 和 `alerts` 的计数
- 两个数据源都在本机
- **代码证据**：`api.go:464-469` `CollectMetrics()` + `SELECT COUNT(1) FROM logs/alerts`

**模块 10：维护操作日志**
- 后端 API：`LogAppend`/`LogList`
- 全部操作本机 SQLite `logs` 表
- **代码证据**：`api.go:381` `INSERT INTO logs(level, message, meta)...` → 写本机 DB
...
- **为什么需要第二台电脑**：
  - 后端 `VNCProxyWS`（`api.go:481`）的核心逻辑是 `net.Dial("tcp", target)`，即向目标 IP:端口发起 TCP 连接
  - 目标机器必须运行 VNC Server（如 TightVNC、RealVNC 等），监听 5900 端口
  - 前端 `remote.html` 通过 `doConnect()` 打开 `vnc.html?target=IP:5900`，由 noVNC 库通过 WebSocket 代理连接到目标机器
  - **如果没有第二台电脑运行 VNC Server，连接必然失败**（`net.Dial` 会返回 connection refused）
- **代码证据**：
  - `api.go:484` `conn, err := net.Dial("tcp", target)` → 连接外部目标机器
  - `remote.html:237` `normalizeVncTarget(ip)` → 默认端口 5900
  - `remote.html:271` `if((protocol||'').toUpperCase()!=='VNC')` → 当前仅支持 VNC 协议
- **测试环境要求**：
  - **电脑 A（后端/观察机）**：运行 `main.go`，打开浏览器访问系统
  - **电脑 B（测试机/模拟无人机）**：安装并启动 VNC Server，确保网络与电脑 A 互通
  - 两台电脑需在同一局域网，或通过 VPN/端口转发互通

---

### 6.3 测试建议

**推荐测试顺序**：先完成 9 个单机模块的测试，最后再准备双机环境测试远程桌面。

| 阶段 | 模块 | 所需环境 |
|------|------|----------|
| 第一阶段 | 1,2,4,5,6,7,8,9,10 | 1 台电脑（运行后端 + 浏览器） |
| 第二阶段 | 3（远程桌面控制） | 2 台电脑（后端机 + VNC Server 目标机） |
