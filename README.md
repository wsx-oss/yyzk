...
**技术栈**：Go + Gin + MySQL + Redis + WebSocket + RAG + RL

**核心特性**：
- **安全认证**：用户注册/登录、会话管理、API Token 认证
- **性能优化**：数据库索引、分页、Redis 分布式限流、统计缓存
- **安全防护**：CORS、SQL 注入防护、输入验证、Panic 自动恢复
- **实时监控**：CPU/内存/磁盘/网络监控、WebSocket 推送、阈值告警
- **智能能力**：LLM 航线规划、CoT 思维链分析、智能巡检、RAG 检索增强
- **并发框架**：统一 Worker Pool（IO/CPU 双池）、优先级调度、超时控制
- **仿真与强化学习**：多实例无人机仿真、任务模板、碰撞规避、RL 训练评估
- **全量 DB 持久化**：备份 SQL、语音、仿真快照、RL 策略、知识库文档全部存储于数据库
