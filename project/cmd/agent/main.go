...
func backgroundPusher(serverURL, agentID string, interval time.Duration, provider DroneDataProvider) {
	...
	batPushURL := baseURL + "/api/battery/push"
	flightPushURL := baseURL + "/api/flight/missions/push"
	...
	for {
		...
		resp, err := client.Post(hwPushURL, "application/json", bytes.NewReader(body))
		if err != nil {
			consecutiveFails++
			backoff = interval * time.Duration(1<<min(consecutiveFails, 4))
			if backoff > maxBackoff {
				backoff = maxBackoff
			}
			log.Printf("[Push] Failed to push to %s: %v (retry in %v)", hwPushURL, err, backoff)
			time.Sleep(backoff)
			continue
		}
		resp.Body.Close()
